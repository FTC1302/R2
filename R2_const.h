const int JOY_BUTTON_X=01;
const int JOY_BUTTON_A=02;
const int JOY_BUTTON_B=03;
const int JOY_BUTTON_Y=04;
const int JOY_BUTTON_LB=07;
const int JOY_BUTTON_RB=08;
const int JOY_BUTTON_LT=05;
const int JOY_BUTTON_RT=06;
const int JOY_BUTTON_START=10;
const int JOY_BUTTON_PAUSE=09;
const int JOY_BUTTON_J1_CLICK=11;
const int JOY_BUTTON_J2_CLICK=12;
const int JOY_BUTTON_TOPHAT_UP= 0;
const int JOY_BUTTON_TOPHAT_UP_RIGHT= 1;
const int JOY_BUTTON_TOPHAT_RIGHT= 2;
const int JOY_BUTTON_TOPHAT_DOWN_RIGHT= 3;
const int JOY_BUTTON_TOPHAT_DOWN= 4;
const int JOY_BUTTON_TOPHAT_DOWN_LEFT= 5;
const int JOY_BUTTON_TOPHAT_LEFT= 6;
const int JOY_BUTTON_TOPHAT_UP_LEFT= 7;

const int JOYSTICK_ERROR=7;


const int BUTTON_CLICK_DELAY=300;

const int HAND_LEFT_BOTTOM_JIE_ANG=5;
const int HAND_LEFT_BOTTOM_GUA_ANG=176;
const int HAND_LEFT_TOP_YUN_ANG=40;
const int HAND_LEFT_TOP_JIE_ANG=56;
const int HAND_LEFT_TOP_GUA_ANG=102;
const int HAND_LEFT_TOP_RENG_ANG=180;

const int HAND_RIGHT_BOTTOM_JIE_ANG=0;
const int HAND_RIGHT_BOTTOM_GUA_ANG=178;
const int HAND_RIGHT_TOP_YUN_ANG=40;
const int HAND_RIGHT_TOP_JIE_ANG=56;
const int HAND_RIGHT_TOP_GUA_ANG=102;
const int HAND_RIGHT_TOP_RENG_ANG=180;

const int HAND_OPEN = 1;
const int HAND_CLOSE = 2;
const int HAND_UP = 3;
const int HAND_DOWN = 4;



const int ARM_CLIP_PARK = 1;
const int ARM_CLIP_HORIZON = 2;
const int ARM_CLIP_RELEASE = 3;
const int ARM_CLIP_CLIP = 4;

const int FLIPPER_UP = 1;
const int FLIPPER_DOWN = 2;

const int ARM_CLIP_PARK_ANG = 180;
const int ARM_CLIP_HORIZON_ANG = 100;
const int ARM_CLIP_RELEASE_ANG = 50;
const int ARM_CLIP_CLIP_ANG = 0;

const int FLAPPER_ANG = 90;
const int FLAPPER_SPEED = -30;


const int ARM_LEFT_MIN=50;
const int ARM_LEFT_MAX=2050;
const int ARM_LEFT_SAFE_POS=500;
const int ARM_LEFT_STOP_FLAPPER_POS=110;
const int ARM_LEFT_OPEN_POS=1000;

const int ARM_RIGHT_MIN=50;
const int ARM_RIGHT_MAX=2050;
const int ARM_RIGHT_SAFE_POS=500;
const int ARM_RIGHT_STOP_FLAPPER_POS=110;
const int ARM_RIGHT_OPEN_POS=1000;

const int ARM_TARGET_NONE = -1;
const int ARM_TARGET_LEVEL1 = 625;
const int ARM_TARGET_LEVEL2 = 1400;
const int ARM_TARGET_LEVEL3 = ARM_LEFT_MAX;


const int SERVO_SPEED_VERY_SLOW	=	1;
const int SERVO_SPEED_SLOW	=	3;
const int SERVO_SPEED_NORMAL	=	5;
const int SERVO_SPEED_FAST	=	7;
const int SERVO_SPEED_VERY_FAST	=	9;
